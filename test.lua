...
for _, oldUi in pairs(targetGui:GetChildren()) do
    if oldUi.Name == "JuggProfileGui" or oldUi.Name == "JuggPremiumGui" then
        oldUi:Destroy()
    end
end
...
local function saveSettings()
    -- Create folder safely if it doesn't already exist
...
local orbitConnection, smoothOrbitConnection, collectConnection, antiModConnection, antiAFKConnection
local orbitAngle, smoothOrbitAngle = 0, 0
local modIds = {3010656315, 4226506845, 7978467591}
...
local function toggleAntiMod(enabled)
    ...
    if enabled then
        antiModConnection = Players.PlayerAdded:Connect(function(p)
            if table.find(modIds, p.UserId) then TeleportService:Teleport(game.PlaceId) end
        end)
    else
...
createToggleRow(JuggPage, "Smooth Tracking Camera", "SmoothOrbit", false, toggleSmoothOrbit)
createToggleRow(JuggPage, "Auto Loot Item Drops", "AutoCollect", false, toggleAutoCollect)
createToggleRow(JuggPage, "Instant Respawn Handler", "AutoRespawn", false, toggleAutoRespawn)

createToggleRow(MiscPage, "Server Hop Anti-Mod", "AntiMod", false, toggleAntiMod)
createToggleRow(MiscPage, "Anti-AFK Keep Connection", "AntiAFK", false, toggleAntiAFK)
createToggleRow(MiscPage, "Optimize Framework (FPS Boost)", "FPSBoost", false, toggleFPSBoost)
...
createKeybindSelector(SettingsPage, "Menu Hide Keybind", "ToggleKey")
